...
#import "ArriveQRCodeScanVC.h"
#import <AVFoundation/AVFoundation.h>
#import "AccountMoneyViewController.h"

@interface ArriveQRCodeScanVC ()<AVCaptureMetadataOutputObjectsDelegate>
//扫描框
...
@implementation ArriveQRCodeScanVC
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"是否确定收货？" message:[NSString stringWithFormat:@"订单号：%@", stringValue] preferredStyle:UIAlertControllerStyleAlert];
...
- (void)accountRequestEvent {
    ...
    NSLog(@"请求参数%@",params);
    [[HttpManage shareInstance] postArriveUrlWithParmaeters:params Success:^(NSMutableDictionary *dic) {
        NSLog(@"收账返回数据：%@",dic);
        NSString * codeStr = [NSString stringWithFormat:@"%@",dic[@"code"]];
        NSString * message = [NSString stringWithFormat:@"%@",dic[@"message"]];
        if ([codeStr isEqualToString:@"1"]) {
            [Utils alertWithMessage:@"收货成功"];
            //创建一个消息对象 刷新交账界面
